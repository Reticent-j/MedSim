...
export const CASES = [
    ...
    stages: [
      {
        ...
        stageIndex: 1,
        triggerOn: ['disp_admit'],
        stageLabel: 'Stage 2 — Cardiogenic Shock',
        transitionMessage: '6 hours after ward admission — Robert becomes unresponsive. Nurses call a code blue. BP is 70/40, HR 130 bpm. He is in cardiogenic shock from a completed anterior MI. The cath lab was never activated.',
        patientUpdate: {
          chiefComplaint: 'Unresponsive — cardiogenic shock following completed anterior STEMI',
          vitals: [
            { label: 'BP',     value: '70/40',    status: 'abnormal' },
            { label: 'HR',     value: '130 bpm',  status: 'abnormal' },
            { label: 'RR',     value: '28 /min',  status: 'abnormal' },
            { label: 'Temp',   value: '36.4 °C',  status: 'normal'   },
            { label: 'O₂ Sat', value: '88%',      status: 'abnormal' },
            { label: 'Pain',   value: '4 / 10',   status: 'borderline' },
          ],
          history: 'Patient admitted 6 hours ago for "chest pain workup" without STEMI protocol activation. Now found unresponsive in bed. Nursing notes document progressive bradycardia and hypotension over 2 hours that went untreated. Anterior MI has completed — the damage is done. This is now a salvage scenario.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_stemi_shock', label: 'Rapid Bedside Assessment', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'Skin mottled, cold, clammy. JVD present. Pulmonary edema. Cardiogenic shock confirmed.' },
              finding: { type: 'exam', title: 'Rapid Bedside Assessment', subtitle: 'Hemodynamic Status',
                results: [
                  { name: 'Skin',        val: 'Mottled, cool, diaphoretic extremities',          flag: 'critical' },
                  { name: 'JVD',         val: 'Marked — elevated to angle of jaw',                flag: 'critical' },
                  { name: 'Lungs',       val: 'Diffuse wet crackles — pulmonary edema',           flag: 'critical' },
                  { name: 'Heart',       val: 'Distant S1/S2, S3 gallop present',                 flag: 'critical' },
                  { name: 'Extremities', val: 'Bilateral pitting edema to knees, no pulses in feet', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Classic cardiogenic shock: cold/wet profile. Left ventricular failure from completed MI. Immediate hemodynamic support and emergent catheterization required.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_echo_shock', label: 'Emergent Bedside Echocardiogram', icon: 'lab', points: 40,
              finding: { type: 'labs', title: 'Point-of-Care Echocardiogram', subtitle: 'Hemodynamic Assessment',
                results: [
                  { name: 'EF',             val: '18% (severely reduced)',       ref: '(55–70%)', flag: 'critical' },
                  { name: 'Anterior wall',  val: 'Akinesis V1–V4 segments',      ref: '',         flag: 'critical' },
                  { name: 'LV dilation',    val: 'Marked LV enlargement',        ref: '',         flag: 'critical' },
                  { name: 'Pericardium',    val: 'Small pericardial effusion',    ref: '',         flag: 'abnormal' },
                ],
                note: { type: 'alert', text: 'EF 18% — cardiogenic shock from completed anterior MI. No mechanical complication (no VSD or papillary rupture). IABP + emergent PCI is last-ditch salvage.' } } },
            { id: 'lab_s2_vasopressors', label: 'Initiate Vasopressors + Inotropes', icon: 'lab', points: 30,
              finding: { type: 'labs', title: 'Hemodynamic Support Orders', subtitle: 'Vasopressors & Inotropes',
                results: [
                  { name: 'Norepinephrine', val: '0.1 mcg/kg/min — MAP target ≥ 65 mmHg',  flag: 'normal'   },
                  { name: 'Dobutamine',     val: '5 mcg/kg/min — improve cardiac output',  flag: 'normal'   },
                  { name: 'Response',       val: 'BP improved to 86/52 mmHg temporarily',  flag: 'abnormal' },
                ],
                note: { type: '', text: 'Vasopressors are a bridge to definitive treatment, not a solution. Emergent PCI with IABP support is the only evidence-based intervention that improves survival.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_cath_shock', label: 'Emergent Interventional Cardiology', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'Cath lab activated. Team mobilizing. Last chance for myocardial salvage with IABP + PCI.' },
              finding: { type: 'consult', title: 'Interventional Cardiology — STAT', subtitle: 'Cardiogenic Shock Protocol',
                results: [
                  { name: 'Assessment',  val: 'Cardiogenic shock — completed anterior STEMI, EF 18%',     flag: 'critical' },
                  { name: 'Plan',        val: 'Emergent PCI + IABP insertion — OR activation underway',    flag: 'normal'   },
                  { name: 'Prognosis',   val: '30-day mortality 40–50% — would have been 5–10% with timely Stage 1 PCI', flag: 'critical' },
                  { name: 'Teaching',    val: 'Every 1-hour delay in STEMI PCI = 1 million cardiomyocytes lost', flag: 'abnormal' },
                ],
                note: { type: 'alert', text: 'Salvage PCI in cardiogenic shock still reduces mortality by ~13% absolute (SHOCK trial). But this outcome was entirely preventable with door-to-balloon within 90 min.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_stemi_iabp', label: 'Emergent PCI + IABP — Cath Lab Now', icon: 'er', outcome: 'correct', points: 150,
            feedback: { title: 'Salvage Attempt — IABP + PCI', grade: 'Salvage',
              body: 'Correct escalation, though this outcome was entirely preventable. The SHOCK trial showed that early revascularization in cardiogenic shock reduces 6-month mortality. IABP provides hemodynamic support during PCI. Robert has a 40–50% chance of survival — compared to 5–10% mortality had Stage 1 PCI occurred on time. This is the best available option now.' } },
          { id: 'disp_s2_stemi_pressors', label: 'Vasopressors Only — Continue Medical Management', icon: 'rx', outcome: 'partial', points: -50,
            feedback: { title: 'Incomplete — Revascularization Required', grade: 'Inadequate',
              body: 'Medical stabilization with vasopressors is a bridge, not definitive therapy. Cardiogenic shock from completed STEMI requires emergency revascularization. Vasopressors alone carry 60–80% in-hospital mortality. The cath lab must be activated.' } },
          { id: 'disp_s2_stemi_comfort', label: 'Goals of Care Conversation — Comfort Measures', icon: 'admit', outcome: 'incorrect', points: -100,
            feedback: { title: 'Premature — Patient Is Salvageable', grade: 'Premature Palliation',
              body: 'While comfort-focused care is appropriate when interventions are futile, this patient has a potentially salvageable situation with emergent PCI + IABP. Transitioning to comfort measures at this stage without attempting revascularization in a 54-year-old deprives him of a meaningful chance of survival. Code status conversations should occur, but emergent cardiology intervention should proceed simultaneously.' } },
        ],
        criticalActions: ['lab_s2_echo_shock', 'consult_s2_cath_shock'],
      },
    ],
  },
  ...
  // ─── ER Case 2: Maya T. — Bacterial Meningitis ────────
  {
    ...
    stages: [
      {
        id: 'stage_2_mening_herniation',
        triggerOn: ['disp_observe'],
        stageLabel: 'Stage 2 — Uncal Herniation',
        transitionMessage: '8 hours after general ward admission — Maya becomes unresponsive. Her pupils are fixed and dilated bilaterally. The nursing staff found her unresponsive in bed. General ward antibiotics were inadequate and delayed. Cerebral edema has now caused transtentorial herniation.',
        patientUpdate: {
          chiefComplaint: 'Unresponsive — cerebral herniation secondary to untreated bacterial meningitis',
          vitals: [
            { label: 'BP',     value: '188/54',   status: 'abnormal' },
            { label: 'HR',     value: '46 bpm',   status: 'abnormal' },
            { label: 'RR',     value: '6 /min',   status: 'abnormal' },
            { label: 'Temp',   value: '40.1 °C',  status: 'abnormal' },
            { label: 'O₂ Sat', value: '84%',      status: 'abnormal' },
            { label: 'GCS',    value: '5 / 15',   status: 'abnormal' },
          ],
          history: 'Maya was admitted to the general ward and given IV antibiotics, but without ICU-level monitoring or dexamethasone. Over 8 hours, cerebral edema progressed unchecked. She is now exhibiting Cushing\'s triad (hypertension, bradycardia, irregular breathing) — a pre-terminal herniation sign. Pupils are blown bilaterally.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_mening_neuro', label: 'Urgent Neurological Assessment', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'GCS 5. Fixed dilated pupils. Decerebrate posturing. Cushing\'s triad. Transtentorial herniation in progress.' },
              finding: { type: 'exam', title: 'Emergency Neurological Assessment', subtitle: 'Herniation Signs',
                results: [
                  { name: 'GCS',             val: '5 (E1V2M2) — profound coma',                   flag: 'critical' },
                  { name: 'Pupils',           val: 'Bilateral fixed & dilated (6 mm) — CN III palsy', flag: 'critical' },
                  { name: 'Motor response',   val: 'Decerebrate posturing (extensor) bilaterally',   flag: 'critical' },
                  { name: 'Breathing',        val: 'Cheyne-Stokes pattern — 6 breaths/min',          flag: 'critical' },
                  { name: 'Cushing\'s triad', val: 'PRESENT — HTN + bradycardia + irregular breathing', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Transtentorial uncal herniation is occurring. Cushing\'s triad = terminal sign of ICP crisis. Immediate intubation, hyperosmolar therapy (mannitol/hypertonic saline), and neurosurgery consultation are required. This outcome was entirely preventable with ICU admission and dexamethasone at Stage 1.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_mening_icp', label: 'Emergent CT Head (No Contrast)', icon: 'lab', points: 25,
              finding: { type: 'labs', title: 'CT Head — Emergency Non-Contrast', subtitle: 'Herniation Protocol',
                results: [
                  { name: 'Cerebral edema',   val: 'Severe diffuse — sulcal effacement',           flag: 'critical' },
                  { name: 'Uncal herniation',  val: 'Present — uncus displaced 5 mm medially',     flag: 'critical' },
                  { name: 'Cisterns',          val: 'Basal cisterns obliterated',                   flag: 'critical' },
                  { name: 'Midline shift',     val: '4 mm rightward shift',                         flag: 'critical' },
                  { name: 'Ventricles',        val: 'Compressed — severe hydrocephalus',            flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Imaging confirms herniation. Neurosurgery must be activated for emergent EVD (external ventricular drain) or decompressive craniectomy. Mannitol 1 g/kg IV NOW while awaiting surgery.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_mening_neuro', label: 'Emergency Neurosurgery', icon: 'consult', points: 35,
              stateEffect: { state: 'improving', message: 'Neurosurgery activated. EVD being placed. Mannitol given. Last-ditch effort at saving Maya.' },
              finding: { type: 'consult', title: 'Emergency Neurosurgery', subtitle: 'Herniation Crisis',
                results: [
                  { name: 'Assessment',  val: 'Transtentorial herniation — ICP crisis from bacterial meningitis', flag: 'critical' },
                  { name: 'Plan',        val: 'Emergent EVD placement + decompressive craniectomy',               flag: 'normal'   },
                  { name: 'Mannitol',    val: '1 g/kg IV bolus immediately — osmotic therapy',                    flag: 'normal'   },
                  { name: 'Dexameth.',   val: 'Should have been started at Stage 1 — now too late',               flag: 'critical' },
                  { name: 'Prognosis',   val: 'Severe disability or death likely — 50–80% mortality at this stage', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Teaching point: Dexamethasone 0.15 mg/kg QID reduces cerebral edema and improves outcomes in bacterial meningitis. IV antibiotics within 30 minutes + ICU monitoring + dexamethasone at Stage 1 would have prevented herniation entirely. Delay in escalation was fatal.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_mening_neuro', label: 'Emergency Neurosurgery — EVD + Decompressive Craniectomy', icon: 'er', outcome: 'correct', points: 100,
            feedback: { title: 'Correct Escalation — But Outcome Avoidable', grade: 'Salvage',
              body: 'Emergency neurosurgical intervention with EVD placement and decompressive craniectomy is the only option at this stage. However, Maya faces 50–80% mortality and near-certain severe neurological disability even if she survives. This outcome was entirely preventable: ICU-level care, IV antibiotics within 30 min of presentation, and dexamethasone at Stage 1 would have prevented herniation.' } },
          { id: 'disp_s2_mening_mannitol_only', label: 'Mannitol + Hyperventilate — Continue Medical Management', icon: 'rx', outcome: 'partial', points: -60,
            feedback: { title: 'Insufficient for Active Herniation', grade: 'Critical Error',
              body: 'Mannitol and hyperventilation are bridge therapies while awaiting surgery — not definitive treatment for herniation. With blown pupils and decerebrate posturing, surgical decompression is mandatory. Medical management alone at this stage is inadequate.' } },
          { id: 'disp_s2_mening_comfort', label: 'Goals of Care — Family Meeting for Comfort Care', icon: 'admit', outcome: 'incorrect', points: -100,
            feedback: { title: 'Premature — Salvage Attempt Required', grade: 'Premature Palliation',
              body: 'While prognosis is poor, Maya is 23 years old and surgical intervention still offers a chance of survival. Goals of care conversation is appropriate, but should not precede or replace an attempt at neurosurgical salvage without family consent and a complete assessment. A 23-year-old with potentially reversible pathology should not be palliated without a surgical consult.' } },
        ],
        criticalActions: ['lab_s2_mening_icp', 'consult_s2_mening_neuro'],
      },
    ],
  },
  ...
  // ─── ER Case 3: Carlos R. — Acute Appendicitis ────────
  {
    ...
    stages: [
      {
        id: 'stage_2_perf_appy',
        stageIndex: 1,
        triggerOn: ['disp_observe_c'],
        stageLabel: 'Stage 2 — Perforated Appendicitis',
        transitionMessage: '48 hours later — Carlos returns by ambulance. He is febrile to 39.4 °C, tachycardic at 138 bpm, and in septic shock. His abdomen is board-like with guarding and rebound throughout. His appendix has perforated.',
        patientUpdate: {
          chiefComplaint: 'Perforated appendicitis with peritonitis — 48 hours after antibiotic-only admission',
          vitals: [
            { label: 'BP',     value: '88/54',    status: 'abnormal' },
            { label: 'HR',     value: '138 bpm',  status: 'abnormal' },
            { label: 'RR',     value: '26 /min',  status: 'abnormal' },
            { label: 'Temp',   value: '39.4 °C',  status: 'abnormal' },
            { label: 'O₂ Sat', value: '94%',      status: 'abnormal' },
            { label: 'Pain',   value: '10 / 10',  status: 'abnormal' },
          ],
          history: 'Carlos was managed with IV antibiotics overnight and discharged after 24 hours with "clinical improvement." He was told to follow up in clinic but collapsed at home 24 hours later. Family called EMS. He arrived in the ED with diffuse peritonitis and septic shock. Non-operative management of appendicitis failed — perforation and periappendiceal abscess are now present on imaging.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_appy_perit', label: 'Repeat Abdominal Examination', icon: 'exam', points: 25,
              stateEffect: { state: 'worsening', message: 'Board-like abdomen. Generalized peritonitis. Septic shock. This patient needs the OR within 1 hour.' },
              finding: { type: 'exam', title: 'Abdominal Examination', subtitle: 'Peritonitis Assessment',
                results: [
                  { name: 'Rigidity',       val: 'Board-like — diffuse involuntary guarding',  flag: 'critical' },
                  { name: 'Rebound',        val: 'Present diffusely — generalized peritonitis', flag: 'critical' },
                  { name: 'Percussion',     val: 'Tympanic RLQ — free air suspected',           flag: 'critical' },
                  { name: 'Bowel sounds',   val: 'Absent — ileus developing',                   flag: 'abnormal' },
                  { name: 'RLQ',            val: 'Point tenderness at McBurney\'s exquisite',   flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Diffuse peritonitis is a surgical emergency. No imaging needed if patient is unstable — take to OR. If stable enough for CT, order immediately to confirm free air and abscess.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_cbc_perf', label: 'Repeat CBC & Blood Cultures', icon: 'lab', points: 20,
              finding: { type: 'labs', title: 'Complete Blood Count — Repeat', subtitle: 'Sepsis Workup',
                results: [
                  { name: 'WBC',         val: '28,400/µL with 94% neutrophils',  ref: '(4.5–11)',  flag: 'critical' },
                  { name: 'Bands',       val: '22% — left shift',                ref: '(<5%)',     flag: 'critical' },
                  { name: 'Lactate',     val: '4.8 mmol/L',                      ref: '(<2)',      flag: 'critical' },
                  { name: 'Creatinine',  val: '1.9 mg/dL',                       ref: '(0.7–1.2)', flag: 'abnormal' },
                  { name: 'Blood Cx ×2', val: 'Pending — drawn and sent',        ref: '',          flag: 'normal'   },
                ],
                note: { type: 'alert', text: 'Sepsis criteria met (SOFA ≥ 2 + suspected infection). Lactate 4.8 = septic shock. IV fluids and broad-spectrum antibiotics NOW. OR preparation in parallel.' } } },
          ] },
          { group: 'Imaging', items: [
            { id: 'img_s2_ct_perf', label: 'CT Abdomen (Perforation Protocol)', icon: 'imaging', points: 30,
              finding: { type: 'imaging', title: 'CT Abdomen & Pelvis', subtitle: 'With Contrast — Perforation Protocol',
                results: [
                  { name: 'Free air',       val: 'Present — pneumoperitoneum subdiaphragmatic',    flag: 'critical' },
                  { name: 'Appendix',       val: 'Not visualized — perforated and collapsed',      flag: 'critical' },
                  { name: 'Abscess',        val: 'Periappendiceal abscess 4.2 × 3.8 cm with air-fluid level', flag: 'critical' },
                  { name: 'Peritoneum',     val: 'Diffuse fat stranding — generalized peritonitis', flag: 'critical' },
                  { name: 'Ascites',        val: 'Moderate turbid free fluid throughout abdomen',  flag: 'abnormal' },
                ],
                note: { type: 'alert', text: 'Free air + periappendiceal abscess confirms perforated appendicitis with generalized peritonitis. Emergency laparotomy required. Mortality 2–5% with prompt surgery; rises dramatically with further delay.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_surgery_perf', label: 'Emergency General Surgery', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'Surgery team activated. OR being prepared. Correct escalation — laparotomy is the only option now.' },
              finding: { type: 'consult', title: 'Emergency General Surgery', subtitle: 'Perforated Appendicitis',
                results: [
                  { name: 'Assessment',   val: 'Perforated appendicitis → generalized peritonitis → septic shock', flag: 'critical' },
                  { name: 'Plan',         val: 'Emergency laparotomy — washout, appendectomy, drain placement',      flag: 'normal'   },
                  { name: 'Pre-op',       val: 'Pip-tazo 4.5 g IV + fluid resuscitation 30 mL/kg + vasopressors if MAP < 65', flag: 'normal' },
                  { name: 'OR ETA',       val: '45 minutes — team mobilizing now',                                  flag: 'normal'   },
                  { name: 'Prognosis',    val: 'Mortality 2–5% with surgery; would have been <0.1% with Stage 1 laparoscopic appendectomy', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Non-operative management of appendicitis fails in 20–30% of cases. The APPAC trial showed 27% failure rate at 5 years. This patient needed surgery at Stage 1 — this outcome was preventable.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_appy_or', label: 'Emergency Laparotomy — OR Now + Broad-Spectrum Antibiotics', icon: 'er', outcome: 'correct', points: 200,
            feedback: { title: 'Correct — Emergency Laparotomy Required', grade: 'Salvage',
              body: 'Emergency laparotomy with peritoneal washout and appendectomy is the only appropriate management for perforated appendicitis with generalized peritonitis and septic shock. Pre-operative antibiotics (pip-tazo), IV fluid resuscitation, and vasopressors are essential adjuncts. Note: this outcome was entirely preventable — laparoscopic appendectomy in Stage 1 carries < 0.1% mortality.' } },
          { id: 'disp_s2_appy_abx_only', label: 'IV Antibiotics Only — Repeat CT in 48 Hours', icon: 'admit', outcome: 'partial', points: -80,
            feedback: { title: 'Inadequate for Generalized Peritonitis', grade: 'Critical Error',
              body: 'Antibiotics-only management is acceptable for uncomplicated appendicitis (Stage 1) — but NOT for perforated appendicitis with generalized peritonitis and septic shock. Free air on CT plus diffuse peritonitis mandates emergency surgery. Further delay risks multi-organ failure. Surgery cannot be deferred here.' } },
          { id: 'disp_s2_appy_discharge', label: 'Drain Abscess Percutaneously — Discharge', icon: 'rx', outcome: 'incorrect', points: -200,
            feedback: { title: 'Inappropriate for Generalized Peritonitis', grade: 'Fatal Error',
              body: 'Percutaneous drainage is appropriate for a LOCALIZED periappendiceal abscess — not for diffuse generalized peritonitis. This patient has free air, diffuse peritoneal contamination, and septic shock. Percutaneous drainage would address only the abscess while leaving fecal contamination throughout the abdomen untreated. Discharge in septic shock is a fatal error.' } },
        ],
        criticalActions: ['img_s2_ct_perf', 'consult_s2_surgery_perf'],
      },
    ],
  },
  ...
  // ─── ER Case 4: Diana L. — Pulmonary Embolism ─────────
  {
    ...
    stages: [
      {
        id: 'stage_2_massive_pe',
        stageIndex: 1,
        triggerOn: ['disp_o2_observe'],
        stageLabel: 'Stage 2 — Massive PE / Obstructive Shock',
        transitionMessage: '3 hours after office observation — Diana collapses. EMS called. On arrival BP 60/palpable, HR 148, O₂ 82% on 15L NRB. Her submassive PE has deteriorated to massive PE with obstructive shock.',
        patientUpdate: {
          chiefComplaint: 'Obstructive shock — massive PE with hemodynamic collapse',
          vitals: [
            { label: 'BP',     value: '60/palpable', status: 'abnormal' },
            { label: 'HR',     value: '148 bpm',     status: 'abnormal' },
            { label: 'RR',     value: '32 /min',     status: 'abnormal' },
            { label: 'Temp',   value: '36.9 °C',     status: 'normal'   },
            { label: 'O₂ Sat', value: '82%',         status: 'abnormal' },
            { label: 'Pain',   value: '9 / 10',      status: 'abnormal' },
          ],
          history: 'Diana was placed on supplemental oxygen in the office and "observed" without anticoagulation. She deteriorated over 3 hours and was found unresponsive by a nurse. EMS brought her emergently. She is now in obstructive shock from massive PE. The right ventricle is failing under acute pressure overload. This is a code-level emergency.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_pe_shock', label: 'Rapid Hemodynamic Assessment', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'Pulsus paradoxus. Neck veins distended. No breath sounds right base. Obstructive shock — RV is collapsing under pressure.' },
              finding: { type: 'exam', title: 'Rapid Cardiopulmonary Assessment', subtitle: 'Obstructive Shock',
                results: [
                  { name: 'JVD',            val: 'Massive — venous pulsations visible',         flag: 'critical' },
                  { name: 'Heart',          val: 'Loud P2 component, RV heave, S3 gallop',       flag: 'critical' },
                  { name: 'Lungs',          val: 'Markedly diminished right base, scattered crackles', flag: 'critical' },
                  { name: 'Skin',           val: 'Cyanotic lips and fingertips, mottled extremities', flag: 'critical' },
                  { name: 'Neuro',          val: 'Decreased LOC — GCS 12 (E3 V4 M5)',           flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Classic obstructive shock: RV failure from massive PE. Treatment target shifts to systemic thrombolysis (tPA) — catheter-directed therapy is NOT appropriate when BP is 60 systolic.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_echo_pe', label: 'Bedside Echocardiogram (Point-of-Care)', icon: 'lab', points: 40,
              finding: { type: 'labs', title: 'Point-of-Care Echocardiogram', subtitle: 'RV Assessment',
                results: [
                  { name: 'RV size',        val: 'Massively dilated — RV > LV (normal RV/LV < 0.6)', ref: '', flag: 'critical' },
                  { name: 'Septal shift',   val: 'D-sign present — IVS bowing into LV in diastole',  ref: '', flag: 'critical' },
                  { name: 'TAPSE',          val: '8 mm (severely impaired)',                          ref: '(≥17 mm normal)', flag: 'critical' },
                  { name: 'IVC',            val: 'Dilated, non-collapsible — elevated CVP',           ref: '', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'RV dilation with D-sign = obstructive shock from massive PE confirmed. The rule of thumb: if BP < 90 systolic + PE confirmed = systemic thrombolysis (alteplase 100 mg over 2h). Catheter-directed therapy not appropriate here — too slow for hemodynamic collapse.' } } },
            { id: 'lab_s2_tpa_assess', label: 'Thrombolysis Eligibility Assessment', icon: 'lab', points: 20,
              finding: { type: 'labs', title: 'Systemic Thrombolysis Assessment', subtitle: 'Contraindication Check',
                results: [
                  { name: 'Recent surgery',   val: 'None within 10 days',                             flag: 'normal'   },
                  { name: 'Intracranial',      val: 'No stroke, tumor, AVM, or recent head trauma',  flag: 'normal'   },
                  { name: 'Active bleeding',   val: 'None identified',                                flag: 'normal'   },
                  { name: 'BP',                val: '60 systolic — meets hemodynamic collapse criteria', flag: 'critical' },
                  { name: 'Conclusion',        val: 'No absolute contraindications — systemic tPA indicated', flag: 'normal' },
                ],
                note: { type: 'alert', text: 'Alteplase 100 mg IV over 2 hours is the treatment of choice for massive PE with hemodynamic collapse. If arrest, give 50 mg IV push.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_pert', label: 'PERT Team + Emergency Medicine', icon: 'consult', points: 35,
              stateEffect: { state: 'improving', message: 'PERT team called. Systemic tPA being prepared. Cath lab on standby for catheter embolectomy if tPA fails.' },
              finding: { type: 'consult', title: 'Pulmonary Embolism Response Team (PERT)', subtitle: 'Massive PE Protocol',
                results: [
                  { name: 'Assessment',    val: 'Massive PE — obstructive shock (SBP 60, O₂ 82%)',                flag: 'critical' },
                  { name: 'Treatment',     val: 'Alteplase 100 mg IV over 2 hours — systemic thrombolysis',        flag: 'normal'   },
                  { name: 'Why not CDT',   val: 'Catheter-directed thrombolysis requires stable hemodynamics — not appropriate here', flag: 'critical' },
                  { name: 'Backup',        val: 'Surgical embolectomy if tPA contraindicated or fails within 60 min', flag: 'normal' },
                  { name: 'Prognosis',     val: 'In-hospital mortality ~25% — would have been < 3% with Stage 1 anticoagulation', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Key teaching: submassive → massive PE threshold. Submassive = RV strain without hemodynamic compromise (anticoagulate ± CDT). Massive = BP < 90 systolic or cardiac arrest (systemic thrombolysis or surgical embolectomy).' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_pe_tpa', label: 'Systemic tPA (Alteplase 100 mg IV) + ICU Admission', icon: 'er', outcome: 'correct', points: 200,
            feedback: { title: 'Correct — Systemic Thrombolysis for Massive PE', grade: 'Salvage',
              body: 'Systemic alteplase is the correct treatment for massive PE with hemodynamic collapse. Catheter-directed therapy (appropriate for submassive PE in Stage 1) is too slow when BP is 60 systolic — the clot burden must be rapidly dissolved. Diana has a ~25% in-hospital mortality now vs. < 3% had she received anticoagulation at Stage 1. This outcome was entirely preventable.' } },
          { id: 'disp_s2_pe_cdt', label: 'Catheter-Directed Thrombolysis (CDT)', icon: 'admit', outcome: 'partial', points: -50,
            feedback: { title: 'Wrong Tool for Hemodynamic Collapse', grade: 'Significant Error',
              body: 'CDT is appropriate for SUBMASSIVE PE (which Diana had in Stage 1). For MASSIVE PE with SBP < 90 systolic, CDT is too slow — the patient will die while you wait for catheter positioning. Systemic thrombolysis with alteplase 100 mg IV over 2 hours is the indicated treatment at this severity level.' } },
          { id: 'disp_s2_pe_observe', label: 'Supplemental O₂ + Observation (Again)', icon: 'rx', outcome: 'incorrect', points: -200,
            feedback: { title: 'Repeated Fatal Error', grade: 'Fatal Error',
              body: 'Observation without anticoagulation or thrombolysis in a patient with massive PE and BP 60 systolic is uniformly fatal. This is the same error that led to Stage 2. Diana is in cardiac arrest within minutes without intervention. Immediate systemic thrombolysis is her only chance of survival.' } },
        ],
        criticalActions: ['lab_s2_echo_pe', 'consult_s2_pert'],
      },
    ],
  },
  ...
  // ─── ER Case 5: Marcus T. — Aortic Dissection ─────────
  {
    ...
    stages: [
      {
        id: 'stage_2_dissect_rupture',
        triggerOn: ['disp_antihtn'],
        stageLabel: 'Stage 2 — Aortic Rupture / Hemorrhagic Shock',
        transitionMessage: '4 hours after office antihypertensive management — Marcus collapses in the waiting area. His BP is now unrecordable. The aorta has ruptured. He is in hemorrhagic shock from an expanding hemopericardium and hemothorax.',
        patientUpdate: {
          chiefComplaint: 'Hemorrhagic shock — ruptured Type A aortic dissection after delayed surgical referral',
          vitals: [
            { label: 'BP',     value: 'Unrecordable',  status: 'abnormal' },
            { label: 'HR',     value: '140 bpm',        status: 'abnormal' },
            { label: 'RR',     value: '32 /min',        status: 'abnormal' },
            { label: 'Temp',   value: '35.9 °C',        status: 'normal'   },
            { label: 'O₂ Sat', value: '80%',            status: 'abnormal' },
            { label: 'GCS',    value: '9 / 15',         status: 'abnormal' },
          ],
          history: 'Marcus was placed on IV esmolol in the office and "monitored" without CT surgery consultation. Over 4 hours, the dissection extended and the false lumen ruptured, causing a massive hemopericardium (cardiac tamponade) and left hemothorax. He is now in PEA arrest territory. The aortic root tear has progressed to rupture.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_dissect_shock', label: 'Rapid Resuscitation Assessment', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'Beck\'s triad: muffled heart sounds, JVD, hypotension. Tracheal deviation left. Hemopericardium + hemothorax. Rupture confirmed.' },
              finding: { type: 'exam', title: 'Emergency Trauma Assessment', subtitle: 'Hemorrhagic Shock',
                results: [
                  { name: 'Heart sounds',  val: 'Muffled — hemopericardium / tamponade',              flag: 'critical' },
                  { name: 'JVD',           val: 'Marked — obstructive physiology',                    flag: 'critical' },
                  { name: 'Trachea',       val: 'Deviated LEFT — massive left hemothorax',            flag: 'critical' },
                  { name: 'Breath sounds', val: 'Absent left — hemothorax confirmed',                 flag: 'critical' },
                  { name: 'Skin',          val: 'Gray, mottled, cold — distributive + obstructive shock', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Beck\'s triad (muffled sounds + JVD + hypotension) + tracheal deviation = cardiac tamponade + hemothorax from aortic rupture. This is an immediate OR situation. Pericardiocentesis is a temporizing measure only — OR is the definitive treatment.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_dissect_xray', label: 'Portable Chest X-Ray + Point-of-Care Echo', icon: 'lab', points: 30,
              finding: { type: 'labs', title: 'Emergency Bedside Imaging', subtitle: 'Tamponade + Hemothorax',
                results: [
                  { name: 'CXR — mediastinum', val: 'Massively widened (>12 cm) — aortic rupture',       flag: 'critical' },
                  { name: 'CXR — left chest',  val: 'Massive left hemothorax — opacified',               flag: 'critical' },
                  { name: 'Echo — pericardium', val: 'Large hemopericardium — diastolic collapse of RV', flag: 'critical' },
                  { name: 'Echo — aortic root', val: 'Aortic root dilated to 5.8 cm — rupture site',    flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Cardiac tamponade + massive hemothorax from aortic rupture. No time for CTA. Activate OR NOW. Without surgery in 30–60 minutes, survival is 0%.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_dissect_cts', label: 'STAT Cardiothoracic Surgery — OR Activation', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'CT Surgery en route. OR activated. Temporary pericardiocentesis buys 10 minutes. Racing against time — this rupture was entirely preventable.' },
              finding: { type: 'consult', title: 'Cardiothoracic Surgery — Emergency Callback', subtitle: 'Aortic Rupture Protocol',
                results: [
                  { name: 'Assessment',  val: 'Ruptured Type A dissection — cardiac tamponade + hemothorax', flag: 'critical' },
                  { name: 'Plan',        val: 'Emergency sternotomy + cardiopulmonary bypass + graft replacement', flag: 'normal' },
                  { name: 'OR ETA',      val: '< 20 minutes — all hands on deck',                                  flag: 'normal' },
                  { name: 'Prognosis',   val: 'Operative mortality now 60–80% (was 10–20% with timely elective repair)', flag: 'critical' },
                  { name: 'Teaching',    val: 'Type A dissection mortality = 1–2% per hour. Every 60 minutes without surgery cost Marcus 1–2% survival', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'This outcome was fully preventable. Type A aortic dissection is always a surgical emergency that cannot be managed conservatively. Office antihypertensive management is contraindicated as a disposition — it is a bridge only while activating the OR.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_dissect_or', label: 'Emergency Sternotomy + Aortic Graft — OR Immediate', icon: 'er', outcome: 'correct', points: 120,
            feedback: { title: 'Correct — Last Chance Surgery', grade: 'Salvage',
              body: 'Emergency sternotomy with graft replacement of the aortic root is the only option. However, Marcus now faces 60–80% operative mortality — compared to 10–20% with timely surgery at Stage 1. Type A dissection mortality increases 1–2% per hour without surgical repair. The 4-hour delay caused by "office monitoring" has drastically reduced his chances of survival.' } },
          { id: 'disp_s2_dissect_pericardio', label: 'Pericardiocentesis + ICU — Stabilize Medically', icon: 'admit', outcome: 'partial', points: -80,
            feedback: { title: 'Temporizing Only — Surgery Required', grade: 'Inadequate',
              body: 'Pericardiocentesis can provide minutes of hemodynamic stabilization but is a bridge, not definitive treatment. The aorta is actively bleeding — no amount of fluid, vasopressors, or pericardiocentesis will stop the hemorrhage. OR activation must happen simultaneously or the patient will die.' } },
          { id: 'disp_s2_dissect_palliative', label: 'Call the Family — Comfort Care', icon: 'home', outcome: 'incorrect', points: -150,
            feedback: { title: 'Premature — Salvage Attempt Required', grade: 'Premature Palliation',
              body: 'Marcus is a 62-year-old with a surgical emergency that offers a real chance of survival with emergency OR intervention. Without attempting surgery, death is certain within minutes. Comfort measures without a surgical attempt are inappropriate given the salvageable nature of this presentation.' } },
        ],
        criticalActions: ['consult_s2_dissect_cts', 'lab_s2_dissect_xray'],
      },
    ],
  },
  ...
  // ─── ER Case 6: Barbara K. — Acute Ischemic Stroke ────
  {
    ...
    stages: [
      {
        id: 'stage_2_stroke_malignant_mca',
        triggerOn: ['disp_aspirin_admit'],
        stageLabel: 'Stage 2 — Malignant MCA Infarction',
        transitionMessage: '36 hours after ward admission without reperfusion — Barbara is found unresponsive. Repeat CT shows massive right MCA territory infarction with 12 mm midline shift. Malignant cerebral edema has developed. Without decompressive craniectomy, she will herniate and die.',
        patientUpdate: {
          chiefComplaint: 'Malignant MCA infarction — cerebral edema with herniation signs',
          vitals: [
            { label: 'BP',     value: '202/110',  status: 'abnormal' },
            { label: 'HR',     value: '56 bpm',   status: 'abnormal' },
            { label: 'RR',     value: '8 /min',   status: 'abnormal' },
            { label: 'Temp',   value: '38.2 °C',  status: 'borderline' },
            { label: 'O₂ Sat', value: '88%',      status: 'abnormal' },
            { label: 'GCS',    value: '6 / 15',   status: 'abnormal' },
          ],
          history: 'Barbara was admitted with aspirin and placed on telemetry. Without tPA or thrombectomy, the MCA territory infarction completed. 1.9 million neurons per minute died during the 36-hour window. Now she has malignant cerebral edema — the most feared complication of large MCA stroke. The 2nd–3rd day post-stroke is the peak edema window.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_stroke_herniation', label: 'Emergency Neurological Exam', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'Fixed and dilated right pupil. Left hemiplegia. GCS 6. Cushing\'s triad. Uncal herniation in progress.' },
              finding: { type: 'exam', title: 'Emergency Neurological Assessment', subtitle: 'Malignant Edema',
                results: [
                  { name: 'GCS',           val: '6 (E1V2M3) — severe obtundation',                 flag: 'critical' },
                  { name: 'Right pupil',   val: 'Fixed, dilated (7 mm) — uncal herniation',        flag: 'critical' },
                  { name: 'Left pupil',    val: 'Reactive (3 mm)',                                  flag: 'abnormal' },
                  { name: 'Motor',         val: 'Complete left hemiplegia, right arm posturing',   flag: 'critical' },
                  { name: 'Cushing\'s',    val: 'PRESENT — HTN + bradycardia + irregular breathing', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Early uncal herniation from malignant MCA infarction. Decompressive hemicraniectomy within 48h of stroke onset reduces mortality by 50% (DESTINY-II trial). Without surgery, mortality is 70–80%.' } } },
          ] },
          { group: 'Imaging', items: [
            { id: 'img_s2_stroke_ct', label: 'Repeat CT Head Non-Contrast', icon: 'imaging', points: 30,
              finding: { type: 'imaging', title: 'CT Head — Day 2', subtitle: 'Malignant Edema Protocol',
                results: [
                  { name: 'Infarct size',   val: 'Complete right MCA territory — >50% hemisphere',    flag: 'critical' },
                  { name: 'Midline shift',  val: '12 mm rightward — herniation threshold crossed',    flag: 'critical' },
                  { name: 'Uncus',          val: 'Displaced — uncal herniation present',               flag: 'critical' },
                  { name: 'Hemorrhage',     val: 'Petechial hemorrhagic transformation',               flag: 'abnormal' },
                  { name: 'Cisterns',       val: 'Partially effaced — elevated ICP confirmed',        flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Malignant MCA infarction. Classic scenario for decompressive hemicraniectomy (>50% MCA territory + <60 years old = strongest indication). DESTINY-II trial: craniectomy improves survival in age 61+ as well, though functional outcomes less favorable.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_stroke_neurosurg', label: 'Emergency Neurosurgery — Hemicraniectomy', icon: 'consult', points: 35,
              stateEffect: { state: 'improving', message: 'Neurosurgery team activated. OR prep for decompressive hemicraniectomy. This was preventable with timely reperfusion at Stage 1.' },
              finding: { type: 'consult', title: 'Neurosurgery Consult — STAT', subtitle: 'Malignant MCA Edema',
                results: [
                  { name: 'Indication',  val: 'Malignant MCA infarction — decompressive hemicraniectomy', flag: 'critical' },
                  { name: 'Timing',      val: 'Must occur within 48h of stroke onset — window closing',    flag: 'critical' },
                  { name: 'Evidence',    val: 'DESTINY-II: NNT=2 for survival, though 50% moderate-severe disability', flag: 'abnormal' },
                  { name: 'Teaching',    val: 'Reperfusion with tPA + thrombectomy at Stage 1 would have prevented this — 100% MCA territory would not have infarcted', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Decompressive hemicraniectomy removes a large skull flap to allow the swollen brain to expand outward rather than downward (herniation). It saves lives but does not restore the infarcted tissue. Functional outcomes range from moderate disability to severe disability.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_stroke_craniectomy', label: 'Emergency Decompressive Hemicraniectomy', icon: 'er', outcome: 'correct', points: 120,
            feedback: { title: 'Correct — Life-Saving Surgery', grade: 'Salvage',
              body: 'Decompressive hemicraniectomy is the correct intervention for malignant MCA infarction with >12 mm midline shift. The DESTINY-II trial shows it reduces 30-day mortality from 70% to 30%. However, Barbara will likely face moderate-to-severe disability. This outcome was preventable — tPA + thrombectomy at Stage 1 would have dissolved the clot before the MCA territory completed infarction.' } },
          { id: 'disp_s2_stroke_mannitol', label: 'Mannitol + Hyperventilate — Medical ICP Management', icon: 'rx', outcome: 'partial', points: -60,
            feedback: { title: 'Bridge Therapy Only — Surgery Required', grade: 'Inadequate',
              body: 'Mannitol and controlled hyperventilation can transiently reduce ICP as a bridge to surgery, but are not definitive treatments for malignant MCA edema with active herniation. Without hemicraniectomy, Barbara will herniate fatally within hours. Surgical decompression is mandatory.' } },
          { id: 'disp_s2_stroke_palliative', label: 'Palliative Care — Withdraw Treatment', icon: 'home', outcome: 'incorrect', points: -80,
            feedback: { title: 'Premature Without Surgical Attempt', grade: 'Significant Error',
              body: 'While prognosis is poor, Barbara is a 71-year-old who may still benefit from hemicraniectomy, which reduces 30-day mortality by 40% absolute. Withdrawing care without attempting surgery bypasses an evidence-based life-saving intervention. Goals of care should be discussed with family in the context of a known surgical option, not used as a substitute for it.' } },
        ],
        criticalActions: ['img_s2_stroke_ct', 'consult_s2_stroke_neurosurg'],
      },
    ],
  },
  ...
  // ─── Ortho Case 3: Alex T. — Compartment Syndrome ────
  {
    ...
    stages: [
      {
        id: 'stage_2_cs_muscle_necrosis',
        triggerOn: ['disp_loosen_cast'],
        stageLabel: 'Stage 2 — Muscle Necrosis / Rhabdomyolysis',
        transitionMessage: '18 hours after cast loosening and observation — the patient is brought back by parents. His forearm is black, firm, and non-tender (sensory loss). Myoglobin from necrotic muscle is flooding his kidneys. Rhabdomyolysis and impending Volkmann\'s contracture are now established.',
        patientUpdate: {
          chiefComplaint: 'Established muscle necrosis — compartment syndrome untreated for 18+ hours',
          vitals: [
            { label: 'BP',     value: '148/92',  status: 'abnormal'   },
            { label: 'HR',     value: '106 bpm', status: 'abnormal'   },
            { label: 'RR',     value: '18 /min', status: 'borderline' },
            { label: 'Temp',   value: '38.3 °C', status: 'borderline' },
            { label: 'O₂ Sat', value: '97%',     status: 'normal'     },
            { label: 'Urine',  value: 'Cola-colored', status: 'abnormal' },
          ],
          history: 'The cast was loosened last night and the patient was observed for 1 hour before discharge. Parents noted he never regained full sensation. He slept through the night and woke unable to move his fingers or feel his forearm. Brought to the ER — forearm is now dusky/black, firm, and the muscles are beginning to liquefy. 18+ hours of ischemia = irreversible muscle death.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_cs_forearm', label: 'Forearm Neurovascular Re-exam', icon: 'exam', points: 15,
              stateEffect: { state: 'worsening', message: 'No sensation, no movement, dusky skin. Muscles necrotic. Volkmann\'s contracture has begun. Fasciotomy is still required but will not restore function — only prevent further systemic damage.' },
              finding: { type: 'exam', title: 'Forearm Neurovascular Examination', subtitle: 'Established Necrosis',
                results: [
                  { name: 'Skin color',    val: 'Dusky purple to black — skin blistering beginning',  flag: 'critical' },
                  { name: 'Sensation',     val: 'ABSENT — complete anesthesia forearm and hand',       flag: 'critical' },
                  { name: 'Motor',         val: 'No active finger or wrist movement — complete paralysis', flag: 'critical' },
                  { name: 'Radial pulse',  val: 'Absent — arterial thrombosis from prolonged compression', flag: 'critical' },
                  { name: 'Compartment',   val: 'Board-hard — no longer tense (liquefying muscle)',   flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Irreversible muscle necrosis (Volkmann\'s ischemic contracture) after 6+ hours of untreated compartment syndrome. Fasciotomy can still prevent further systemic complications (renal failure from rhabdomyolysis) but cannot restore hand or forearm function. The window for limb salvage was Stage 1.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_cs_rhab', label: 'Rhabdomyolysis / AKI Panel', icon: 'lab', points: 30,
              finding: { type: 'labs', title: 'Rhabdomyolysis Panel', subtitle: 'Renal + Muscle Markers',
                results: [
                  { name: 'CK',           val: '142,000 U/L',   ref: '(< 200)',       flag: 'critical' },
                  { name: 'Myoglobin',    val: '18,400 ng/mL',  ref: '(< 90)',        flag: 'critical' },
                  { name: 'Creatinine',   val: '3.8 mg/dL',     ref: '(0.7–1.2)',     flag: 'critical' },
                  { name: 'BUN',          val: '68 mg/dL',      ref: '(7–20)',        flag: 'critical' },
                  { name: 'Potassium',    val: '6.2 mEq/L',     ref: '(3.5–5.0)',     flag: 'critical' },
                  { name: 'Urine',        val: 'Myoglobinuria — cola-colored, dipstick "blood" without RBCs', ref: '', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Severe rhabdomyolysis with acute kidney injury. Aggressive IV fluid resuscitation (Lactated Ringer\'s 1–2 L/hr) is required to prevent myoglobin precipitation in tubules. Target urine output > 200 mL/hr. Monitor for hyperkalemia-induced cardiac arrhythmia.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_cs_ortho', label: 'Orthopedic Surgery — Fasciotomy + Debridement', icon: 'consult', points: 35,
              stateEffect: { state: 'improving', message: 'OR activated. Fasciotomy + debridement of necrotic muscle will prevent systemic sepsis and renal failure. Hand function is permanently lost.' },
              finding: { type: 'consult', title: 'Orthopedic Surgery', subtitle: 'Late Compartment Syndrome + Rhabdomyolysis',
                results: [
                  { name: 'Assessment',    val: 'Established Volkmann\'s ischemic contracture — irreversible',       flag: 'critical' },
                  { name: 'Plan',          val: 'Fasciotomy + debridement of necrotic muscle + wound VAC',           flag: 'normal'   },
                  { name: 'Limb salvage',  val: 'Salvage of limb anatomy possible, but hand function permanently lost', flag: 'critical' },
                  { name: 'Amputation',    val: 'If infection develops — may ultimately require amputation',         flag: 'abnormal' },
                  { name: 'Teaching',      val: 'Window for limb salvage is 4–6 hours. This patient had untreated compartment syndrome for 18+ hours.', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Emergency fasciotomy at Stage 1 would have taken 20 minutes and restored full hand and forearm function. The 18-hour delay has resulted in permanent motor and sensory loss. Rhabdomyolysis management is now the priority to prevent renal failure.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_cs_fasciotomy_debride', label: 'Emergency Fasciotomy + Debridement + Aggressive IV Fluids', icon: 'er', outcome: 'correct', points: 120,
            feedback: { title: 'Correct — Prevents Systemic Complications', grade: 'Salvage',
              body: 'Late fasciotomy with debridement of necrotic muscle prevents sepsis, myoglobin-induced renal failure, and further limb complications. Aggressive IV fluid therapy (LR 1–2 L/hr with urine output monitoring) prevents AKI from rhabdomyolysis. However, the hand and forearm function are permanently lost — this outcome was entirely preventable with emergency fasciotomy at Stage 1.' } },
          { id: 'disp_s2_cs_fluids_only', label: 'IV Fluids Only — Rhabdomyolysis Protocol (No Surgery)', icon: 'admit', outcome: 'partial', points: -80,
            feedback: { title: 'Incomplete — Surgery Still Required', grade: 'Critical Error',
              body: 'Aggressive fluid therapy addresses the rhabdomyolysis but leaves necrotic tissue in situ, risking myonecrotic sepsis and persistent systemic muscle breakdown. Fasciotomy with debridement of dead muscle is essential to stop the source of myoglobin and prevent life-threatening infection from liquefying muscle.' } },
          { id: 'disp_s2_cs_amputation', label: 'Primary Forearm Amputation', icon: 'rx', outcome: 'incorrect', points: -80,
            feedback: { title: 'Premature — Attempt Limb Salvage First', grade: 'Premature',
              body: 'Primary amputation without an attempt at fasciotomy and debridement is premature. Many cases of late compartment syndrome with partial necrosis can be salvaged with aggressive debridement, even if full function is not restored. Amputation is the last resort after failed salvage attempts or life-threatening systemic sepsis.' } },
        ],
        criticalActions: ['lab_s2_cs_rhab', 'consult_s2_cs_ortho'],
      },
    ],
  },
  ...
  // ─── ER Case 9: Ludwig's Angina ───────────────────────
  {
    ...
    stages: [
      {
        id: 'stage_2_ludwigs_obstruction',
        triggerOn: ['disp_ludwigs_abx_only'],
        stageLabel: 'Stage 2 — Progressive Airway Obstruction',
        transitionMessage: '12 hours after floor admission with IV antibiotics — the nurse calls a rapid response. The patient is sitting bolt upright, drooling, unable to swallow. Stridor has developed. The airway lumen has narrowed to < 3 mm. He is minutes from complete obstruction.',
        patientUpdate: {
          chiefComplaint: 'Impending complete airway obstruction — Ludwig\'s Angina not controlled by antibiotics alone',
          vitals: [
            { label: 'BP',     value: '152/96',  status: 'abnormal'   },
            { label: 'HR',     value: '128 bpm', status: 'abnormal'   },
            { label: 'RR',     value: '32 /min', status: 'abnormal'   },
            { label: 'Temp',   value: '39.8 °C', status: 'abnormal'   },
            { label: 'O₂ Sat', value: '89%',     status: 'abnormal'   },
            { label: 'Stridor', value: 'Present', status: 'abnormal'  },
          ],
          history: 'Patient was placed on IV piperacillin-tazobactam and admitted to a general floor without ENT or anesthesia consultation. Over 12 hours, the bilateral sublingual and submandibular infections have progressed. The tongue has elevated to the roof of the mouth. Severe trismus prevents direct laryngoscopy. The patient is sitting forward in the "tripod position" and cannot swallow his secretions.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_ludwigs_airway', label: 'Emergency Airway Assessment', icon: 'exam', points: 20,
              stateEffect: { state: 'worsening', message: 'Stridor + tripod position + SpO₂ 89%. Airway minutes from complete obstruction. No RSI — trismus prevents laryngoscopy. Awake fiberoptic is the ONLY option.' },
              finding: { type: 'exam', title: 'Emergency Airway Assessment', subtitle: 'Impending Obstruction',
                results: [
                  { name: 'Stridor',       val: 'Audible at 6 feet — severe inspiratory stridor',        flag: 'critical' },
                  { name: 'Posture',       val: 'Tripod position — unable to lie flat',                   flag: 'critical' },
                  { name: 'Trismus',       val: 'Severe — jaw opening < 1 cm (was 2 cm at Stage 1)',     flag: 'critical' },
                  { name: 'Tongue',        val: 'Elevated to soft palate — minimal visible oropharynx',  flag: 'critical' },
                  { name: 'Floor of mouth',val: 'Woody, indurated, rising — sublingual pushing tongue up', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'AIRWAY EMERGENCY. Standard rapid sequence intubation is CONTRAINDICATED — distorted anatomy + trismus makes direct laryngoscopy impossible. Awake fiberoptic intubation by experienced operator, or surgical airway (cricothyrotomy) if that fails. Do NOT attempt blind RSI.' } } },
          ] },
          { group: 'Imaging', items: [
            { id: 'img_s2_ludwigs_ct_neck', label: 'CT Neck with Contrast (If Stable)', icon: 'imaging', points: 20,
              finding: { type: 'imaging', title: 'CT Neck — Emergency', subtitle: 'Airway Lumen Assessment',
                results: [
                  { name: 'Airway lumen',   val: '< 3 mm at base of tongue — critical stenosis',           flag: 'critical' },
                  { name: 'Sublingual',     val: 'Bilateral gas-forming phlegmon expanding rapidly',        flag: 'critical' },
                  { name: 'Submandibular',  val: 'Bilateral abscess formation — surgical drainage required', flag: 'critical' },
                  { name: 'Mediastinum',    val: 'Early tracking — superior mediastinitis developing',      flag: 'critical' },
                ],
                note: { type: 'alert', text: 'DO NOT send this patient to CT if he is not already intubated. A < 3 mm airway lumen can close completely at any moment. Mediastinal extension has begun — surgical drainage is now more urgent than ever.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_ludwigs_anest', label: 'Emergency Anesthesia + ENT (Awake Fiberoptic)', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'Anesthesia and ENT bedside with fiberoptic scope. Last chance for controlled airway — if this fails, surgical airway is the backup.' },
              finding: { type: 'consult', title: 'Emergency Anesthesia + ENT', subtitle: 'Airway Rescue',
                results: [
                  { name: 'Assessment',    val: 'Impending complete airway obstruction — < 3 mm lumen',     flag: 'critical' },
                  { name: 'Plan',          val: 'Awake fiberoptic intubation with ketamine/midazolam sedation', flag: 'normal'   },
                  { name: 'Backup',        val: 'Bedside surgical airway kit open — cricothyrotomy on standby', flag: 'normal'   },
                  { name: 'Surgical',      val: 'Bilateral surgical drainage + debridement in OR after intubation', flag: 'normal' },
                  { name: 'Teaching',      val: 'Airway first, always. This complication was preventable with awake fiberoptic intubation at Stage 1 before trismus worsened.', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Ludwig\'s Angina is one of few conditions where airway management takes absolute precedence before any other intervention. Stage 1 should have secured the airway before any antibiotic trial in an awake, controllable patient. Now the window has nearly closed.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_ludwigs_awake_fob', label: 'Awake Fiberoptic Intubation → ICU → Bilateral Drainage', icon: 'er', outcome: 'correct', points: 150,
            feedback: { title: 'Correct — Airway Secured', grade: 'Salvage',
              body: 'Awake fiberoptic intubation by an experienced anesthesiologist with ENT on standby is the correct approach. Once intubated, bilateral submandibular and sublingual surgical drainage in the OR is essential. However, mediastinitis has begun tracking — mortality has increased from < 5% (Stage 1) to 15–30% with mediastinal involvement.' } },
          { id: 'disp_s2_ludwigs_rsi', label: 'Rapid Sequence Intubation (Video Laryngoscopy)', icon: 'rx', outcome: 'partial', points: -80,
            feedback: { title: 'High-Risk — May Fail in Distorted Anatomy', grade: 'Dangerous Approach',
              body: 'RSI with video laryngoscopy in Ludwig\'s Angina is extremely high risk. Trismus, elevated tongue, and distorted anatomy make successful intubation unlikely and carrying a high "can\'t intubate, can\'t oxygenate" risk. If RSI fails, the patient cannot be mask-ventilated and will arrest. Awake fiberoptic is the standard of care in this presentation.' } },
          { id: 'disp_s2_ludwigs_continue_abx', label: 'Continue IV Antibiotics — Close Monitoring', icon: 'admit', outcome: 'incorrect', points: -200,
            feedback: { title: 'Fatal — Complete Obstruction Imminent', grade: 'Fatal Error',
              body: 'Continuing antibiotics without securing the airway in a patient with stridor, SpO₂ 89%, and a 3 mm airway lumen is fatal. The infection will progress faster than antibiotics can work in woody phlegmon. Complete obstruction, respiratory arrest, and death will follow within 30–60 minutes. Airway must be secured NOW.' } },
        ],
        criticalActions: ['consult_s2_ludwigs_anest', 'pe_s2_ludwigs_airway'],
      },
      {
        id: 'stage_3_ludwigs_mediastinitis',
        triggerOn: ['disp_s2_ludwigs_continue_abx'],
        stageLabel: 'Stage 3 — Descending Necrotizing Mediastinitis',
        transitionMessage: '6 hours after continued antibiotic-only management — complete airway obstruction has occurred. EMS performed a field cricothyrotomy. Patient survived but is now intubated via surgical airway. CT shows descending necrotizing mediastinitis to the carina. This is one of the deadliest complications in ENT.',
        patientUpdate: {
          chiefComplaint: 'Post-cricothyrotomy — descending necrotizing mediastinitis, septic shock',
          vitals: [
            { label: 'BP',     value: '74/48',    status: 'abnormal' },
            { label: 'HR',     value: '142 bpm',  status: 'abnormal' },
            { label: 'RR',     value: '22 /min (ventilated)', status: 'abnormal' },
            { label: 'Temp',   value: '40.2 °C',  status: 'abnormal' },
            { label: 'O₂ Sat', value: '91% (on vent)', status: 'abnormal' },
            { label: 'GCS',    value: 'Intubated / Sedated', status: 'abnormal' },
          ],
          history: 'Complete airway obstruction occurred at home. EMS performed an emergency cricothyrotomy — the patient barely survived. Infection has tracked down the fascial planes into the mediastinum. CT shows gas and fluid from the neck to the bifurcation. Descending necrotizing mediastinitis (DNM) carries 25–50% mortality even with optimal management. This is the direct consequence of not securing the airway and performing drainage at Stage 1.',
        },
        actions: [
          { group: 'Labs', items: [
            { id: 'lab_s3_ludwigs_sepsis', label: 'Sepsis Bundle + CT Chest/Neck', icon: 'lab', points: 25,
              finding: { type: 'labs', title: 'Sepsis Panel + Mediastinitis Imaging', subtitle: 'DNM Assessment',
                results: [
                  { name: 'WBC',        val: '38,000/µL with 96% neutrophils',    ref: '(4.5–11)',   flag: 'critical' },
                  { name: 'Lactate',    val: '5.8 mmol/L',                        ref: '(< 2)',      flag: 'critical' },
                  { name: 'Procalciton',val: '112 ng/mL',                         ref: '(< 0.5)',    flag: 'critical' },
                  { name: 'CT medias.', val: 'Descending gas + fluid to carina — Endo-Martos Grade IIb', ref: '', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Descending necrotizing mediastinitis (DNM) — one of the most feared complications in head and neck surgery. Requires bilateral neck drainage + trans-sternal mediastinal debridement + prolonged ICU care. Mortality 25–50%.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s3_ludwigs_cts', label: 'Cardiothoracic Surgery + ENT — Mediastinal Debridement', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'CT Surgery and ENT joint OR: trans-sternal mediastinal debridement + bilateral neck drainage. This is a 6-hour operation with 25-50% mortality.' },
              finding: { type: 'consult', title: 'Cardiothoracic Surgery + ENT Joint Consult', subtitle: 'Descending Necrotizing Mediastinitis',
                results: [
                  { name: 'Plan',       val: 'Trans-sternal + bilateral cervical debridement + washout',   flag: 'normal'   },
                  { name: 'Duration',   val: '5–8 hours — major operative undertaking',                    flag: 'abnormal' },
                  { name: 'ICU',        val: 'Post-op ventilation ×7–14 days expected',                    flag: 'abnormal' },
                  { name: 'Mortality',  val: '25–50% despite optimal treatment',                           flag: 'critical' },
                  { name: 'Teaching',   val: 'Securing airway + bilateral drainage at Stage 1 would have prevented ALL of this with < 1% mortality', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'DNM is a preventable complication. Had the airway been secured and bilateral drainage performed at Stage 1, the infection would not have spread to the mediastinum. Each stage of delay geometrically worsened the prognosis.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s3_ludwigs_trans_sternal', label: 'Trans-Sternal Mediastinal Debridement + Prolonged ICU', icon: 'er', outcome: 'correct', points: 100,
            feedback: { title: 'Correct — Last Salvage Option', grade: 'Salvage',
              body: 'Trans-sternal mediastinal debridement with bilateral neck drainage and prolonged ICU management is the only viable approach. Despite optimal care, mortality is 25–50%. If this patient survives, he faces weeks of ICU care, multiple reoperations, and prolonged recovery. Compare this to the Stage 1 outcome: awake fiberoptic intubation + bilateral drainage = < 1% mortality and likely discharge in 7–10 days.' } },
          { id: 'disp_s3_ludwigs_palliative', label: 'Goals of Care — Comfort Focused Management', icon: 'admit', outcome: 'partial', points: -30,
            feedback: { title: 'Contextually Appropriate — But Surgery Still Viable', grade: 'Acceptable if Family Declines',
              body: 'With 25–50% mortality and a prolonged, painful ICU course, goals of care discussions are entirely appropriate. If the patient or family declines aggressive surgical management after being fully informed, comfort-focused care is a valid choice. However, surgery should be offered first as it provides a meaningful chance of survival.' } },
          { id: 'disp_s3_ludwigs_abx_only_again', label: 'IV Antibiotics + Percutaneous Drain — Avoid Surgery', icon: 'rx', outcome: 'incorrect', points: -200,
            feedback: { title: 'Fatal — Mediastinitis Cannot Be Drained Percutaneously', grade: 'Fatal Error',
              body: 'Descending necrotizing mediastinitis from gas-forming organisms requires open surgical debridement. Percutaneous drainage cannot adequately address the complex, multiloculated infected fascial planes in the mediastinum. Antibiotic-only management of mediastinitis is uniformly fatal. Surgery is mandatory.' } },
        ],
        criticalActions: ['lab_s3_ludwigs_sepsis', 'consult_s3_ludwigs_cts'],
      },
    ],
  },
  ...
  // ─── ER Case 13: Pheochromocytoma Crisis ─────────────
  {
    ...
    stages: [
      {
        id: 'stage_2_pheo_crisis_labetolol',
        triggerOn: ['disp_pheo_labetolol'],
        stageLabel: 'Stage 2 — Unopposed Alpha Crisis / Hypertensive Emergency',
        transitionMessage: 'Within 20 minutes of IV labetalol — the patient\'s BP has spiked to 300/160. She is seizing. Labetalol blocked beta-receptors (vasodilation), leaving catecholamine-driven alpha vasoconstriction completely unopposed. This is a known, predictable, and fatal complication of beta-blockade in pheochromocytoma.',
        patientUpdate: {
          chiefComplaint: 'Hypertensive emergency (300/160) + seizure — unopposed alpha crisis after beta-blockade',
          vitals: [
            { label: 'BP',     value: '300/160',  status: 'abnormal' },
            { label: 'HR',     value: '48 bpm',   status: 'abnormal' },
            { label: 'RR',     value: '24 /min',  status: 'abnormal' },
            { label: 'Temp',   value: '38.6 °C',  status: 'abnormal' },
            { label: 'O₂ Sat', value: '91%',      status: 'abnormal' },
            { label: 'GCS',    value: '8 / 15',   status: 'abnormal' },
          ],
          history: 'IV labetalol was given for "hypertensive emergency." Within 20 minutes, BP surged to 300/160 mmHg. The beta-blockade abolished compensatory vasodilation, leaving peripheral alpha-adrenergic vasoconstriction from tumor-secreted norepinephrine completely unopposed — a classic textbook catastrophe. The patient is now convulsing. This is a medical emergency caused by the wrong treatment.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_pheo_crisis', label: 'Emergency Assessment — Seizure Protocol', icon: 'exam', points: 15,
              stateEffect: { state: 'worsening', message: 'Tonic-clonic seizure. BP 300/160. Hypertensive encephalopathy. Fundoscopy: papilledema and flame hemorrhages. This is the direct result of beta-blockade before alpha-blockade in pheochromocytoma.' },
              finding: { type: 'exam', title: 'Emergency Neurological & Cardiovascular Assessment', subtitle: 'Catecholamine Crisis',
                results: [
                  { name: 'BP',           val: '300/160 mmHg — hypertensive emergency',              flag: 'critical' },
                  { name: 'Neuro',        val: 'Tonic-clonic seizure — hypertensive encephalopathy', flag: 'critical' },
                  { name: 'Fundoscopy',   val: 'Papilledema + flame hemorrhages — grade IV HTN retinopathy', flag: 'critical' },
                  { name: 'Cardiac',      val: 'Bradycardia (labetalol) + severe hypertension',      flag: 'critical' },
                  { name: 'Diaphoresis',  val: 'Profuse — ongoing catecholamine surge',              flag: 'abnormal' },
                ],
                note: { type: 'alert', text: 'BETA-BLOCKADE BEFORE ALPHA-BLOCKADE IN PHEO = FATAL COMPLICATION. Labetalol blocks beta-2 vasodilation, leaving alpha-1 mediated vasoconstriction unopposed. BP rises precipitously. Immediate IV phentolamine (alpha-blocker) is the only reversal agent.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_pheo_troponin_crisis', label: 'Troponin / BNP / CT Head', icon: 'lab', points: 25,
              finding: { type: 'labs', title: 'End-Organ Damage Assessment', subtitle: 'Catecholamine Crisis',
                results: [
                  { name: 'Troponin',     val: '6.2 ng/mL',     ref: '(< 0.04)',    flag: 'critical' },
                  { name: 'BNP',          val: '1,840 pg/mL',   ref: '(< 100)',     flag: 'critical' },
                  { name: 'CT head',       val: 'Bilateral posterior white matter changes — PRES', ref: '', flag: 'critical' },
                  { name: 'Creatinine',   val: '3.1 mg/dL',     ref: '(0.7–1.2)',   flag: 'critical' },
                ],
                note: { type: 'alert', text: 'PRES (Posterior Reversible Encephalopathy Syndrome) from extreme hypertension. Catecholamine cardiomyopathy (Takotsubo-like) from massive norepinephrine surge. Multi-organ damage is occurring in real time.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_pheo_endo_crisis', label: 'Emergency Endocrinology + IV Phentolamine', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'IV phentolamine given. BP responding. Seizure controlled with benzodiazepines. Endocrinology confirms pheochromocytoma crisis from iatrogenic beta-blockade.' },
              finding: { type: 'consult', title: 'Emergency Endocrinology — Pheo Crisis', subtitle: 'Unopposed Alpha Emergency',
                results: [
                  { name: 'Diagnosis',   val: 'Pheochromocytoma — unopposed alpha crisis from labetalol',          flag: 'critical' },
                  { name: 'Treatment',   val: 'IV phentolamine 5 mg bolus — alpha blockade reverses the crisis',   flag: 'normal'   },
                  { name: 'Seizure',     val: 'IV lorazepam 4 mg — controlled, PRES management',                   flag: 'normal'   },
                  { name: 'Rule',        val: 'ALPHA FIRST, THEN BETA — always. Never beta-block a pheo first.',  flag: 'critical' },
                  { name: 'Prognosis',   val: 'Cardiac damage (Troponin 6.2) + PRES — prolonged ICU recovery',    flag: 'critical' },
                ],
                note: { type: 'alert', text: 'The cardinal rule of pheochromocytoma management: alpha-blockade (phenoxybenzamine or phentolamine) MUST be established before any beta-blockade. Beta-blockers alone precipitate hypertensive crisis by removing compensatory beta-2 vasodilation.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_pheo_phentolamine', label: 'IV Phentolamine + ICU + Seizure Control + Endocrinology', icon: 'er', outcome: 'correct', points: 120,
            feedback: { title: 'Correct Rescue — Reverses Iatrogenic Crisis', grade: 'Salvage',
              body: 'IV phentolamine (alpha-blocker) is the correct antidote to the iatrogenic unopposed alpha crisis from labetalol. Combined with seizure control (benzodiazepines) and ICU admission, this can reverse the crisis. However, the patient has sustained significant cardiac damage (Troponin 6.2) and PRES. Full recovery is not certain. The rule is absolute: alpha block FIRST in pheochromocytoma.' } },
          { id: 'disp_s2_pheo_more_labetolol', label: 'Increase Labetalol — BP Too High', icon: 'rx', outcome: 'incorrect', points: -200,
            feedback: { title: 'Fatal — Worsens Unopposed Alpha Crisis', grade: 'Fatal Error',
              body: 'Adding more labetalol to treat BP 300/160 in an unopposed alpha crisis will cause further BP elevation, not reduction. This is the fundamental pharmacology error: more beta-blockade = more unopposed alpha = higher BP. Only IV phentolamine (alpha-blocker) will lower the BP in this scenario.' } },
          { id: 'disp_s2_pheo_nitroprusside', label: 'IV Nitroprusside — Hypertensive Emergency', icon: 'admit', outcome: 'partial', points: -40,
            feedback: { title: 'Second-Line — Phentolamine is Superior', grade: 'Suboptimal',
              body: 'IV nitroprusside can lower BP in pheochromocytoma crisis and is a reasonable second-line option. However, phentolamine directly addresses the mechanism (alpha blockade) and is the preferred agent. Nitroprusside without addressing the ongoing catecholamine surge may lead to fluctuating BP and does not prevent recurrent surges from the tumor.' } },
        ],
        criticalActions: ['consult_s2_pheo_endo_crisis', 'lab_s2_pheo_troponin_crisis'],
      },
    ],
  },
  ...
  // ─── ER Case 18: Acute Mesenteric Ischemia ────────────
  {
    ...
    stages: [
      {
        id: 'stage_2_ami_bowel_necrosis',
        triggerOn: ['disp_ami_observe'],
        stageLabel: 'Stage 2 — Transmural Bowel Necrosis / Septic Shock',
        transitionMessage: '12 hours of "NPO + IV fluids + serial exams" — the patient is now in septic shock. The abdomen is silent and board-like. CTA shows full-thickness bowel necrosis from duodenum to mid-transverse colon. The window for bowel salvage has passed.',
        patientUpdate: {
          chiefComplaint: 'Transmural bowel necrosis + fecal peritonitis — delayed surgical management of SMA embolus',
          vitals: [
            { label: 'BP',     value: '72/44',    status: 'abnormal' },
            { label: 'HR',     value: '148 bpm',  status: 'abnormal' },
            { label: 'RR',     value: '30 /min',  status: 'abnormal' },
            { label: 'Temp',   value: '40.0 °C',  status: 'abnormal' },
            { label: 'O₂ Sat', value: '88%',      status: 'abnormal' },
            { label: 'Pain',   value: '10 / 10',  status: 'abnormal' },
          ],
          history: 'Patient admitted 12 hours ago with confirmed SMA embolus but managed conservatively with fluids and serial exams. Pain escalated rather than improved. Bowel sounds disappeared entirely. The intestinal mucosa, then submucosa, then full bowel wall has progressively necrotized. Bacterial translocation has caused gram-negative septic shock. The bowel is now dead from Treitz to mid-transverse colon — requiring massive resection.',
        },
        actions: [
          { group: 'Physical Exam', items: [
            { id: 'pe_s2_ami_peritonitis', label: 'Abdominal Re-examination', icon: 'exam', points: 15,
              stateEffect: { state: 'worsening', message: 'Silent abdomen. Board-like rigidity. Feculent odor. Transmural necrosis has caused fecal peritonitis. 6–8 liters of bowel resection now required for survival — with likely permanent short bowel syndrome.' },
              finding: { type: 'exam', title: 'Abdominal Examination', subtitle: 'Fecal Peritonitis',
                results: [
                  { name: 'Bowel sounds',  val: 'Absent throughout — paralytic ileus',                  flag: 'critical' },
                  { name: 'Rigidity',      val: 'Board-like generalized — fecal peritonitis',            flag: 'critical' },
                  { name: 'Rebound',       val: 'Diffuse — entire abdomen tender',                      flag: 'critical' },
                  { name: 'Distension',    val: 'Severe — pneumoperitoneum',                            flag: 'critical' },
                  { name: 'Rectal',        val: 'Gross blood — full-thickness ischemia confirmed',      flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Fecal peritonitis from transmural bowel necrosis. Emergency laparotomy is the only option. Expected resection: jejunum to mid-transverse colon = > 200 cm loss → permanent short bowel syndrome. TPN dependency for life.' } } },
          ] },
          { group: 'Labs', items: [
            { id: 'lab_s2_ami_sepsis', label: 'Repeat Lactate / CTA / CBC', icon: 'lab', points: 25,
              finding: { type: 'labs', title: 'Repeat Labs — Necrosis Panel', subtitle: 'Septic Shock Assessment',
                results: [
                  { name: 'Lactate',      val: '11.4 mmol/L',   ref: '(< 2)',       flag: 'critical' },
                  { name: 'WBC',          val: '42,000/µL',     ref: '(4.5–11)',    flag: 'critical' },
                  { name: 'pH',           val: '7.08',          ref: '(7.35–7.45)', flag: 'critical' },
                  { name: 'CTA bowel',    val: 'No enhancement duodenum to mid-transverse colon — full-thickness necrosis', ref: '', flag: 'critical' },
                  { name: 'Free air',     val: 'Pneumoperitoneum — perforation',                        flag: 'critical' },
                ],
                note: { type: 'alert', text: 'Lactate 11.4 = profound cellular asphyxia. pH 7.08 = severe lactic acidosis. Pneumoperitoneum = bowel perforation. This patient has minutes to OR. Mortality now > 80%.' } } },
          ] },
          { group: 'Consults', items: [
            { id: 'consult_s2_ami_vasc_surgery', label: 'Emergency Vascular + General Surgery', icon: 'consult', points: 40,
              stateEffect: { state: 'improving', message: 'Joint vascular + general surgery OR. Embolectomy + massive small bowel resection. Patient will likely survive with permanent short bowel syndrome and TPN dependency.' },
              finding: { type: 'consult', title: 'Vascular + General Surgery Emergency', subtitle: 'Bowel Necrosis Resection',
                results: [
                  { name: 'Plan',        val: 'SMA embolectomy + small bowel/right colon resection + damage control', flag: 'normal'   },
                  { name: 'Expected',    val: 'Resection from Treitz to mid-transverse colon → < 100 cm small bowel remaining', flag: 'critical' },
                  { name: 'Short bowel', val: 'Permanent short bowel syndrome — TPN dependency likely for life', flag: 'critical' },
                  { name: 'Mortality',   val: '> 80% (was 20–35% with timely Stage 1 embolectomy)',                flag: 'critical' },
                  { name: 'Teaching',    val: 'SMA embolus + 6h ischemia = reversible. > 12h = irreversible necrosis. Every hour matters in AMI.', flag: 'critical' },
                ],
                note: { type: 'alert', text: 'The "golden period" for mesenteric ischemia is < 6 hours. Stage 1 embolectomy (endovascular or open) restores perfusion before irreversible necrosis. By 12 hours, all that can be done is damage-control surgery with massive resection.' } } },
          ] },
        ],
        dispositions: [
          { id: 'disp_s2_ami_surgery_resect', label: 'Emergency Laparotomy — SMA Embolectomy + Bowel Resection', icon: 'er', outcome: 'correct', points: 100,
            feedback: { title: 'Correct — Damage Control Surgery', grade: 'Salvage',
              body: 'Emergency laparotomy with SMA embolectomy and massive bowel resection is the only option. The patient faces > 80% mortality and, if surviving, permanent short bowel syndrome requiring lifelong TPN. This catastrophic outcome was entirely preventable with timely embolectomy at Stage 1 — when the bowel was viable, the window was open, and mortality was 20–35%.' } },
          { id: 'disp_s2_ami_comfort', label: 'Palliative Care — Withdraw Aggressive Treatment', icon: 'admit', outcome: 'partial', points: -20,
            feedback: { title: 'Contextually Reasonable — But Surgery Still Possible', grade: 'Acceptable',
              body: 'With > 80% mortality, pH 7.08, and lactate 11.4, palliative care is a reasonable discussion. If the patient or family declines surgery given the poor prognosis, comfort-focused management is appropriate and ethically valid. However, surgery should be presented as an option before transitioning to palliation.' } },
          { id: 'disp_s2_ami_more_fluids', label: 'More Aggressive Fluids + Vasopressors — Continue ICU Management', icon: 'rx', outcome: 'incorrect', points: -200,
            feedback: { title: 'Fatal — Dead Bowel Cannot Be Resuscitated Medically', grade: 'Fatal Error',
              body: 'Medical management cannot treat transmural bowel necrosis. Necrotic bowel is a source of ongoing gram-negative bacteremia, endotoxin release, and fecal peritonitis. Without surgical removal of the dead bowel, the patient will die from multi-organ failure within hours regardless of fluid volume or vasopressor doses.' } },
        ],
        criticalActions: ['lab_s2_ami_sepsis', 'consult_s2_ami_vasc_surgery'],
      },
    ],
  },
...
stages: [
  {
    id: 'stage_2_cdiff_toxic_megacolon',
    triggerOn: ['disp_38_incorrect'],
    stageLabel: 'Stage 2 — Toxic Megacolon / Fulminant C. diff',
    transitionMessage: '72 hours without treatment — the patient is brought to the ER by ambulance. Abdomen distended to 15 cm. BP 78/44, HR 142. CT shows transverse colon dilated to 9 cm with intramural gas. Toxic megacolon from untreated C. difficile colitis — perforation imminent.',
    patientUpdate: {
      chiefComplaint: 'Toxic megacolon — fulminant C. difficile from untreated infection',
      vitals: [
        { label: 'BP',     value: '78/44',    status: 'abnormal' },
        { label: 'HR',     value: '142 bpm',  status: 'abnormal' },
        { label: 'RR',     value: '28 /min',  status: 'abnormal' },
        { label: 'Temp',   value: '39.6 °C',  status: 'abnormal' },
        { label: 'O₂ Sat', value: '91%',      status: 'abnormal' },
        { label: 'Pain',   value: '10 / 10',  status: 'abnormal' },
      ],
      history: 'Patient discharged with no treatment 3 days ago. C. difficile colitis has progressed to fulminant colitis with toxic megacolon. The pseudomembranes have eroded through the mucosa. Colonic ileus has developed — the colon is paralyzed and filling with gas and toxin. At 9 cm transverse colon diameter, perforation risk is > 50%. This is a surgical emergency.',
    },
    actions: [
      { group: 'Physical Exam', items: [
        { id: 'pe_s2_cdiff_abdomen', label: 'Abdominal Examination', icon: 'exam', points: 15,
          stateEffect: { state: 'worsening', message: 'Tympanic, distended abdomen. Absent bowel sounds. Point tenderness over transverse colon. Perforation risk > 50%. This patient needs emergency colectomy.' },
          finding: { type: 'exam', title: 'Abdominal Examination', subtitle: 'Toxic Megacolon',
            results: [
              { name: 'Distension',   val: 'Severe — abdominal girth 15 cm above baseline',          flag: 'critical' },
              { name: 'Bowel sounds', val: 'Absent — paralytic ileus',                                flag: 'critical' },
              { name: 'Percussion',   val: 'Tympanic throughout — gaseous distension',               flag: 'critical' },
              { name: 'Tenderness',   val: 'Diffuse with maximal point at transverse colon',          flag: 'critical' },
              { name: 'Rigidity',     val: 'Not yet — but peritoneal signs developing',               flag: 'abnormal' },
            ],
            note: { type: 'alert', text: 'Toxic megacolon: colon diameter > 6 cm + systemic toxicity. Tranverse colon 9 cm = immediate perforation risk. Emergency colectomy is the definitive treatment. Oral vancomycin + IV metronidazole as a bridge while OR is activated.' } } },
      ] },
      { group: 'Labs', items: [
        { id: 'lab_s2_cdiff_ct', label: 'CT Abdomen (Megacolon Protocol)', icon: 'lab', points: 25,
          finding: { type: 'labs', title: 'CT Abdomen & Pelvis', subtitle: 'Toxic Megacolon Assessment',
            results: [
              { name: 'Transverse colon', val: '9.2 cm — severe dilation (normal < 6 cm)',         flag: 'critical' },
              { name: 'Intramural gas',   val: 'Pneumatosis coli — wall necrosis beginning',        flag: 'critical' },
              { name: 'Perforation',      val: 'No free air YET — perforation imminent',            flag: 'critical' },
              { name: 'Pseudomembranes',  val: 'Thickened wall with "accordion sign"',              flag: 'critical' },
              { name: 'Lactate',          val: '5.2 mmol/L',                                        flag: 'critical' },
            ],
            note: { type: 'alert', text: 'Pneumatosis coli = intramural gas from ischemic necrosis — surgery within hours or perforation. Oral vancomycin 500 mg QID + IV fidaxomicin + surgical consult simultaneously.' } } },
      ] },
      { group: 'Consults', items: [
        { id: 'consult_s2_cdiff_surgery', label: 'Emergency General Surgery — Subtotal Colectomy', icon: 'consult', points: 40,
          stateEffect: { state: 'improving', message: 'OR activated. Subtotal colectomy with diverting ileostomy. This colectomy was entirely preventable with 10 days of oral vancomycin at Stage 1.' },
          finding: { type: 'consult', title: 'Emergency General Surgery', subtitle: 'Toxic Megacolon from C. diff',
            results: [
              { name: 'Plan',      val: 'Subtotal colectomy + end ileostomy — emergent',                flag: 'normal'   },
              { name: 'OR ETA',    val: '< 1 hour — perforation imminent',                              flag: 'critical' },
              { name: 'Mortality', val: '30–50% with surgical intervention; nearly 100% without',       flag: 'critical' },
              { name: 'Teaching',  val: 'Oral vancomycin 10-day course at Stage 1 = > 90% cure rate. This colostomy was entirely preventable.', flag: 'critical' },
            ],
            note: { type: 'alert', text: 'C. diff toxic megacolon mortality: 30–50% with colectomy, ~100% without. The IDSA guidelines recommend subtotal colectomy for toxic megacolon in C. diff. The rectum is typically spared to allow future reversal.' } } },
      ] },
    ],
    dispositions: [
      { id: 'disp_s2_cdiff_colectomy', label: 'Emergency Subtotal Colectomy + IV Vancomycin + ICU', icon: 'er', outcome: 'correct', points: 100,
        feedback: { title: 'Correct — Surgical Emergency', grade: 'Salvage',
          body: 'Emergency subtotal colectomy with end ileostomy is the only appropriate management for toxic megacolon with pneumatosis coli and imminent perforation. Pre-operative IV vancomycin and ICU-level resuscitation are adjuncts. Mortality is 30–50% — compare this to > 90% cure rate with a 10-day course of oral vancomycin at Stage 1.' } },
      { id: 'disp_s2_cdiff_oral_vanc_only', label: 'Oral Vancomycin + NG Decompression — Defer Surgery', icon: 'admit', outcome: 'partial', points: -60,
        feedback: { title: 'Too Late for Medical Management Alone', grade: 'Inadequate',
          body: 'Oral vancomycin is appropriate for mild C. diff, but not for toxic megacolon with pneumatosis coli. The 9 cm transverse colon with intramural gas has > 50% perforation risk. Medical management alone at this stage has > 80% mortality. Surgery must proceed alongside any antibiotic therapy.' } },
      { id: 'disp_s2_cdiff_iv_metro', label: 'IV Metronidazole Only — Continue Non-Surgical', icon: 'rx', outcome: 'incorrect', points: -150,
        feedback: { title: 'Insufficient — Surgery Required', grade: 'Critical Error',
          body: 'IV metronidazole alone is insufficient for fulminant C. diff with toxic megacolon. IV metronidazole achieves inadequate intraluminal concentrations in a paralytic colon. Surgery is mandatory at this stage. Without colectomy, perforation and death are the expected outcomes.' } },
    ],
    criticalActions: ['lab_s2_cdiff_ct', 'consult_s2_cdiff_surgery'],
  },
],
},
...
stages: [
  {
    id: 'stage_2_mening2_herniation',
    triggerOn: ['disp_39_incorrect'],
    stageLabel: 'Stage 2 — Brain Herniation / Septic Shock',
    transitionMessage: '16 hours after discharge — EMS brings the patient back unresponsive. Parents report he never improved and stopped responding to voice. GCS is now 6. Pupils are unequal. Bacterial meningitis has progressed to cerebral herniation and bacteremia-driven septic shock.',
    patientUpdate: {
      chiefComplaint: 'Bacterial meningitis — cerebral herniation + septic shock after inadequate treatment',
      vitals: [
        { label: 'BP',     value: '190/50',   status: 'abnormal' },
        { label: 'HR',     value: '52 bpm',   status: 'abnormal' },
        { label: 'RR',     value: '7 /min',   status: 'abnormal' },
        { label: 'Temp',   value: '40.3 °C',  status: 'abnormal' },
        { label: 'O₂ Sat', value: '82%',      status: 'abnormal' },
        { label: 'GCS',    value: '6 / 15',   status: 'abnormal' },
      ],
      history: 'Patient discharged and sent home with a diagnosis of "viral meningitis." Bacterial meningitis continued to worsen unchecked. Cerebral edema progressed over 16 hours. He is now in Cushing\'s triad (hypertension + bradycardia + irregular breathing) — a pre-terminal sign of transtentorial herniation. Neisseria meningitidis has also seeded the bloodstream causing DIC and septic shock.',
    },
    actions: [
      { group: 'Physical Exam', items: [
        { id: 'pe_s2_mening2_emergency', label: 'Emergency Neurological Assessment', icon: 'exam', points: 20,
          stateEffect: { state: 'worsening', message: 'GCS 6. Left pupil blown. Cushing\'s triad. Petechiae have become purpura fulminans. Septic shock + herniation simultaneously.' },
          finding: { type: 'exam', title: 'Emergency Assessment', subtitle: 'Herniation + Septic Shock',
            results: [
              { name: 'GCS',           val: '6 (E1V2M3) — coma',                               flag: 'critical' },
              { name: 'Left pupil',    val: 'Fixed, dilated 7 mm — uncal herniation',           flag: 'critical' },
              { name: 'Right pupil',   val: 'Reactive 3 mm',                                    flag: 'abnormal' },
              { name: 'Posturing',     val: 'Decerebrate (extensor) — brainstem compression',   flag: 'critical' },
              { name: 'Rash',          val: 'Purpura fulminans — large areas of skin necrosis', flag: 'critical' },
              { name: 'BP',            val: '190/50 + HR 52 — Cushing\'s triad',               flag: 'critical' },
            ],
            note: { type: 'alert', text: 'Simultaneous cerebral herniation + meningococcal septic shock (purpura fulminans/DIC). This is a near-unsurvivable scenario. IV ceftriaxone + dexamethasone + emergency neurosurgery + ICU resuscitation — all simultaneously. Outcome is very poor.' } } },
      ] },
      { group: 'Labs', items: [
        { id: 'lab_s2_mening2_panel', label: 'Emergency Labs + CT Head', icon: 'lab', points: 25,
          finding: { type: 'labs', title: 'Emergency Panel', subtitle: 'Herniation + DIC Assessment',
            results: [
              { name: 'CT head',    val: 'Diffuse edema — uncal herniation — cisterns obliterated', flag: 'critical' },
              { name: 'Platelets',  val: '22 × 10³/µL — DIC (was 88 at Stage 1)',                  flag: 'critical' },
              { name: 'D-dimer',    val: '> 40 µg/mL — severe consumptive coagulopathy',           flag: 'critical' },
              { name: 'Fibrinogen', val: '0.6 g/L — critical low (normal > 2.0)',                  flag: 'critical' },
              { name: 'Lactate',    val: '7.8 mmol/L — profound septic shock',                     flag: 'critical' },
            ],
            note: { type: 'alert', text: 'Purpura fulminans = DIC from meningococcemia. Fibrinogen critically low — active hemorrhage into skin and organs. CT confirms herniation. Simultaneously: IV ceftriaxone 2g, dexamethasone 10 mg, mannitol 1 g/kg, FFP + platelets for DIC.' } } },
      ] },
      { group: 'Consults', items: [
        { id: 'consult_s2_mening2_neuro', label: 'Emergency Neurosurgery + ICU + ID', icon: 'consult', points: 30,
          stateEffect: { state: 'improving', message: 'Neurosurgery, ICU, and ID consulted simultaneously. EVD placed. Antibiotics running. DIC management started. Outcome is still very uncertain.' },
          finding: { type: 'consult', title: 'Multi-team Emergency Response', subtitle: 'Herniation + Meningococcal Sepsis',
            results: [
              { name: 'Neurosurgery', val: 'EVD placement — emergent ICP monitoring and drainage', flag: 'normal'   },
              { name: 'ID',           val: 'Ceftriaxone 2g IV q12h + dexamethasone — started now', flag: 'normal'   },
              { name: 'ICU',          val: 'Vasopressors for septic shock + DIC management (FFP, platelets, cryoprecipitate)', flag: 'normal' },
              { name: 'Prognosis',    val: '> 70% mortality — severe disability in survivors',    flag: 'critical' },
              { name: 'Teaching',     val: 'IV antibiotics within 30 min of presentation at Stage 1 would have prevented herniation and DIC entirely.', flag: 'critical' },
            ],
            note: { type: 'alert', text: 'Teaching point: In bacterial meningitis, every 1-hour delay in antibiotics increases mortality by approximately 10%. Early IV antibiotics + dexamethasone at Stage 1 are the two most evidence-based interventions. This outcome is directly attributable to the Stage 1 discharge error.' } } },
      ] },
    ],
    dispositions: [
      { id: 'disp_s2_mening2_icu_full', label: 'ICU — Antibiotics + EVD + DIC Management', icon: 'er', outcome: 'correct', points: 100,
        feedback: { title: 'Correct — Maximal Salvage Attempt', grade: 'Salvage',
          body: 'Simultaneous ICU admission, IV ceftriaxone + dexamethasone, emergency EVD for ICP management, and DIC treatment with FFP/cryoprecipitate/platelets is the correct approach. However, with herniation and purpura fulminans, mortality exceeds 70% and survivors face severe neurological disability. IV antibiotics within 30 minutes of first presentation at Stage 1 would have prevented this outcome entirely.' } },
      { id: 'disp_s2_mening2_abx_only', label: 'IV Antibiotics + General Ward — Monitor Closely', icon: 'admit', outcome: 'incorrect', points: -150,
        feedback: { title: 'Insufficient — ICU + Neurosurgery Required Immediately', grade: 'Critical Error',
          body: 'A patient with GCS 6, herniation signs, and purpura fulminans cannot be managed on a general ward. This patient requires ICU admission, emergency neurosurgery (EVD), and DIC management simultaneously. General ward admission without these interventions will result in death within hours.' } },
      { id: 'disp_s2_mening2_comfort', label: 'Palliative Care — Too Late to Intervene', icon: 'rx', outcome: 'partial', points: -30,
        feedback: { title: 'Premature — Aggressive Attempt Still Warranted', grade: 'Premature Palliation',
          body: 'While prognosis is very poor, aggressive intervention still offers a chance of survival. Without attempting EVD placement, antibiotics, and DIC management, death is certain. Palliative care conversations are appropriate but should not precede a full resuscitation attempt, especially since the patient\'s age and pre-morbid status suggest salvageability with maximal intervention.' } },
    ],
    criticalActions: ['lab_s2_mening2_panel', 'consult_s2_mening2_neuro'],
  },
],
},
